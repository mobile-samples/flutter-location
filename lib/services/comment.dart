...
class FilmCommentThreadClient implements CommentThreadService {
  ...
  Future<int> update(String commentId, String comment, String author) async{
    final headers = await HttpHelper.instance.buildHeader();
    final response = await http.patch( Uri.parse('$url/commentthread/$commentId/$author'),
      headers: headers,
      body: jsonEncode(
          <String, dynamic>{"comment" : comment}),
    );
...
class FilmCommentThreadReplyClient implements CommentThreadReplyService {
  ...
  Future<int> reply(
      String id, String commentId, String comment) async {
    final storage = new FlutterSecureStorage();
    final userId = await storage.read(key: 'userId');
    final headers = await HttpHelper.instance.buildHeader();
    final response = await http.post(
  ...
  Future<int> delete( String commentThreadId, String commentId, String author) async {
    final headers = await HttpHelper.instance.buildHeader();
    final response = await http.delete(Uri.parse("$url/commentthread/$commentThreadId/reply/$commentId/$author"),
      headers: headers,
      body: jsonEncode(
          <String, dynamic>{}),
    );
  ...
  Future<int> update(String commentId, String comment, String author) async {
    final headers = await HttpHelper.instance.buildHeader();
    final response = await http.patch( Uri.parse('$url/commentthread/reply/$commentId/$author'),
      headers: headers,
      body: jsonEncode(
          <String, dynamic>{'comment' : comment}),
    );
  ...
  Future<List<CommentReply>> getComments(String id, String? author) async {
    ...
    if (response.statusCode == 200) {
      ...
      List<CommentReply> listReply = List<CommentReply>.from(
          data.map((x) => CommentReply.fromJson(x)));
      return listReply;
    } else {
      throw json.decode(response.body)['error']['message'];
    }
...
abstract class CommentThreadReplyService {
  Future<List<CommentReply>> getComments(String commentThreadId, String? author) ;
