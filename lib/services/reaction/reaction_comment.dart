...
class ReactionCommentClient implements ReactionService {
  ...
  static final ReactionService instance =
  ReactionCommentClient._instantiate();
  ...
  Future<int> removeUseful(String id, String author, String? userId) async {
    ...
    final response = await http.delete(Uri.parse('$url/commentthread/$id/$author/useful/${userId ?? ''}'), headers: headers);
  ...
  Future<int> setUseful(String id, String author, String? userId) async {
    ...
    final response = await http.post(Uri.parse('$url/commentthread/$id/$author/useful/${userId ?? ''}'),
...
class ReactionRateClient implements ReactionService {
  ...
  Future<int> removeUseful(String id, String author, String? userId) async {
    final headers = await HttpHelper.instance.buildHeader();
    final response = await http.delete(Uri.parse('$url/rates/$id/$author/useful/${userId ?? ''}'), headers: headers);
    if (response.statusCode == 200 && json.decode(response.body) > 0) {
      return 1;
    } else {
      throw json.decode(response.body)['error']['message'];
  ...
  Future<int> setUseful(String id, String author, String? userId) async {
    ...
    final response = await http.post(Uri.parse('$url/rates/$id/$author/useful/${userId ?? ''}'),
        headers: headers, body: jsonEncode(<dynamic, dynamic>{}));
    if (response.statusCode == 200 && json.decode(response.body) > 0) {
      return 1;
    } else {
