...
abstract class RateService {
  Future<int> postRate(
      String id, int rate, String review, String time, bool anonymous);
}

abstract class RateCommentService {
  Future<List<RateReply>> getComments(String locationId, String authorOfRate);
...
class RateReplyClient implements RateCommentService {
  ...
  Future<List<RateReply>> getComments(
    ...
    final response = await http
        .get(Uri.parse('$url/rates/$locationId/$authorOfRate/comments'));
    if (response.statusCode == 200) {
  ...
  Future<int> delete(
    ...
    final response = await http.delete(
      Uri.parse("$url/rates/$commentThreadId/comments/$commentId/$author"),
      headers: headers,
      body: jsonEncode(<String, dynamic>{}),
    );
    if (response.statusCode == 200) {
  ...
  Future<int> update(String commentId, String comment, String author) async {
    ...
    final response = await http.patch(
      Uri.parse('$url/rates/comments/$commentId/$author'),
      headers: headers,
